...
int r_binary_search(int *array, int len, int num, int left, int right, int middle) {
    if (right == -1) right = len-1; 
    int out=-1;
    int *ptr = array;
        if (*(ptr+left)<num && *(ptr+right)>num && (right-left)>1) {
            middle = (right + left + 1) / 2;
            if (num > *(array+middle))
                out = r_binary_search(array, len, num, middle, right, middle);
            else
                out = r_binary_search(array, len, num, left, middle, middle);
        }
        else {
            if (out != -1) return out;
            if (*(array+left) == num || *(array+right) == num) 
...
int main()
{
    ...
    printf("binary_search result: %d\n", r_binary_search(array, len, num, 0, -1, 0));
