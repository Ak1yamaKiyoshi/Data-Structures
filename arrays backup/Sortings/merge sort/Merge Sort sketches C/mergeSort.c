...
void merge(int *array, int low, int mid, int high) {
    int *left = (int*)calloc(sizeof(int), mid-low);
    memcpy(left, &array[low], sizeof(int)*(mid-low));
    int *right = (int*)calloc(sizeof(int), high-mid);
    ...
    while (low + i < mid && mid + j < high) 
        if (left[i] <= right[j]) 
            ...
            array[k++] = right[j++];
    if 
        (low + i < mid) memcpy(&array[k], &left[i], sizeof(int)*(high-k));
    else 
        memcpy(&array[k], &right[j], sizeof(int)*(high-k));
